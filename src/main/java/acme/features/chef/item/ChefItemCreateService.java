...
public class ChefItemCreateService implements AbstractCreateService<Chef, Item>{
	...
	public Item instantiate(final Request<Item> request) {
		...
		inventor = this.repository.findChefById(request.getPrincipal().getActiveRoleId());
		result = new Item();
		result.setName("");
		result.setCode("");
	...
	public void validate(final Request<Item> request, final Item entity, final Errors errors) {
		...
		if (!errors.hasErrors("retailPrice")) {
			...
			errors.state(request, validateCurrency , "retailPrice", "chef.invention.form.error.retailPrice-notAllowed");
			errors.state(request, entity.getRetailPrice().getAmount()>0, "retailPrice", "chef.invention.form.error.retailPrice-amountNotAllowed");
		}
